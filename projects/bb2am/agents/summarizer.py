...
class Summarizer:
    ...
    def buildCorpus(self, context_lines):
        corpus = ""
    ...
    def seperateSentences(self, corpus):
        sentences =  re.split(r'User:|AI:' , corpus)
        sentences = list(filter(None, sentences))
        sentences = [ 'User:' + sentence if i %2 == 0 else 'AI:' + sentence for i, sentence in enumerate(sentences)]
        return sentences
...
class GusumSummarizer(Summarizer):
    ...
    def summarize(self, corpus, sentences = None):
        ...
        if not self.processed:
            corpus = self.buildCorpus(corpus)
    ...
    def _createSummary(self, sentences, sentencesRank, sentenceAmount):
        ...
        for i in range(len(summarySentencesIndexes)):
            ...
            # if "User:" in next_sentence or "AI:" in next_sentence:
            #     summary=summary + '\n ' + next_sentence
            # else:
